...
countData=data.frame(read.table("counts.rna.sva_corrected",header=T,sep='\t'))
...
colnames(confidenceMatt)<-colNameEntries
colnames(foldChangeMat)<-colNameEntries

# Write diffMat to disk
write.table(diffMat,file = "rna.sva_corrected.diffMat.tsv",sep='\t')
write.table(confidenceMat,file = "rna.sva_corrected.confidenceMat.tsv",sep='\t')
write.table(fcMat,file = "rna.sva_corrected.fcMat.tsv",sep='\t')