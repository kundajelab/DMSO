...
data=as.matrix(data)
unique_names=rownames(data)
#take the rlog transform
data=rlogTransformation(data) 
rownames(data)=unique_names
...
fit <- lmFit(data_corrected, mod_final)
fit_with_exp<-lmFit(data_corrected_exp,mod_final)
...
e2=eBayes(fit2)
fit3=contrasts.fit(fit_with_exp,cont.matrix)
e3=eBayes(fit3)
plotSA(e2," SVA-subtracted from Input, Mean-Variance Trend ")
plotSA(e3,"exp(SVA) subtracted from input, Mean-Variance Trend")
...
for(i in c(1,2,3,4,5,6,7,8,9))
{
  ...
  write.table(tab,file=paste("rna_differential_limma_rlog_subtracted",comparisons[i],".tsv",sep=""),quote=TRUE,sep='\t',row.names = FALSE)
}
...
for(i in c(1,2,3,5,6,7,8,9))
{
  ...
  write.table(tab,file=paste("rna_differential_limma_exp_rlog_subtracted",comparisons[i],".tsv",sep=""),quote=TRUE,sep='\t',row.names = FALSE)
}
