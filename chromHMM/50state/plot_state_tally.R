rm(list=ls())
data=read.table("50_all_states.tally.txt",header=TRUE,sep='\t')
data$State=factor(data$State,levels=data$State) 
#colors=read.table("25_state_map.txt",header=FALSE,sep='\t')
#colors$V2=colors$V2/255
#colors$V3=colors$V3/255
#colors$V4=colors$V4/255
#colors=colors[c(1,2,3,5,6,8,9,11,13,14,15,16,17,18,19),]
library(ggplot2)
library(reshape2)
melted=melt(data)
...
p1=ggplot(data=melted,aes(x=variable,y=value,group=State,fill=State))+
  ...
  theme_bw(20)+
  ggtitle("50 Chromatin State Distribution for Differential ATAC-seq Peaks")+
