rm(list=ls())
library(gplots)
data=read.table('vegf_pathway_diff_genes_sup4.csv',header=TRUE,sep='\t',row.names = 1)
data=as.matrix(data)
hmcols<-colorRampPalette(c("blue","white","red"))(23)
...
heatmap.2(data,
          ...
          margins=c(5,10),
          breaks=quantile(data,probs=seq(0,1,0.04))[2:25])
